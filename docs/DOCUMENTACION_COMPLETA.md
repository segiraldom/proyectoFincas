...
- **Mobile**: Arquitectura Flutter con servicios separados
...
| sqflite | 2.3.3 | Plugin SQLite para Flutter |
| http | 1.2.1 | Cliente HTTP para API calls |
| connectivity_plus | 6.0.3 | Detección de estado de conexión |
| uuid | 4.4.0 | Generación de identificadores únicos |
...
│   │   │   ├── 📄 home_screen.dart
│   │   │   ├── 📄 finca_list_screen.dart
│   │   │   ├── 📄 finca_detail_screen.dart
│   │   │   ├── 📄 create_finca_screen.dart
│   │   │   └── 📄 create_activity_screen.dart
│   │   ├── 📁 services/         # Servicios y lógica
│   │   │   ├── 📄 api_service.dart           # Comunicación API
│   │   │   ├── 📄 sync_service.dart          # Sincronización
│   │   │   ├── 📄 finca_local_service.dart   # CRUD local fincas
│   │   │   ├── 📄 actividad_local_service.dart # CRUD local actividades
│   │   │   └── 📄 propietario_local_service.dart # CRUD local propietarios
│   │   ├── 📁 models/           # Modelos de datos
│   │   │   └── 📄 propietario_model.dart
...
### Descripción
Aplicación móvil multiplataforma desarrollada con **Flutter** que permite gestión de fincas desde dispositivos móviles con capacidad offline.

### Archivos Principales

#### `lib/main.dart` - Punto de Entrada
**Propósito**: Inicializar la aplicación Flutter.

**Funcionalidad**:
- Configura MaterialApp con tema verde
- Establece HomeScreen como pantalla inicial
- Desactiva banner de debug

#### `lib/screens/home_screen.dart`
**Propósito**: Pantalla principal de navegación.
...
#### `lib/screens/finca_list_screen.dart`
**Propósito**: Listado de todas las fincas.

**Funcionalidad**:
- Muestra fincas en lista o tarjetas
- Búsqueda y filtrado
- Navegación a detalle de finca
- Opciones de creación/edición

#### `lib/screens/finca_detail_screen.dart`
**Propósito**: Detalle de una finca específica.

**Funcionalidad**:
- Muestra información completa de la finca
- Lista actividades asociadas
- Muestra propietarios
- Opciones de edición y eliminación

#### `lib/screens/create_finca_screen.dart`
**Propósito**: Formulario para crear nueva finca.

**Funcionalidad**:
- Campos: nombre, departamento, municipio, área, coordenadas
- Selección múltiple de propietarios
- Validación de datos
- Guardado local o envío a API

#### `lib/screens/create_activity_screen.dart`
**Propósito**: Formulario para registrar actividad.

**Funcionalidad**:
- Campos: tipo, descripción, cantidad, unidad, producción
- Selección de finca asociada
- Fecha automática
- Guardado local o envío a API
...
**Propósito**: Comunicación con API REST backend.
...
**Métodos Principales**:
- `getFincas()`: Obtener todas las fincas
- `getFinca(id)`: Obtener finca específica
- `createFinca(data)`: Crear nueva finca
- `updateFinca(id, data)`: Actualizar finca
- `deleteFinca(id)`: Eliminar finca
- Métodos similares para actividades y propietarios

#### `lib/services/sync_service.dart`
**Propósito**: Sincronización entre datos locales y servidor.

**Funcionalidad**:
1. **Detección de Conexión**: Usa `connectivity_plus` para verificar internet
2. **Sincronización Pendiente**:
   - Obtiene registros locales no sincronizados
   - Los envía a la API en orden (propietarios → fincas → actividades)
   - Marca como sincronizados los exitosos
3. **Manejo de Errores**: Reintenta en caso de falla
...
Future<void> syncPendingData() async {
  ...
  // 2. Sincronizar propietarios pendientes
  final propietarios = await propietarioLocalService.getPendingPropietarios();
  for (var propietario in propietarios) {
    final success = await apiService.syncPropietario(propietario);
    if (success) {
      await propietarioLocalService.markAsSynced(propietario.id);
    }
  }
  
  // 3. Sincronizar fincas pendientes
  // 4. Sincronizar actividades pendientes
}
```

#### `lib/services/finca_local_service.dart`
**Propósito**: CRUD local de fincas usando SQLite.

**Funcionalidad**:
- `insertFinca(fincas)`: Guardar finca localmente
- `getFinca(id)`: Obtener finca por ID
- `getAllFincas()`: Listar todas las fincas locales
- `getPendingFincas()`: Obtener fincas no sincronizadas
- `updateFinca(id, data)`: Actualizar finca local
- `deleteFinca(id)`: Eliminar finca local
- `markAsSynced(id)`: Marcar como sincronizada

#### `lib/services/actividad_local_service.dart`
**Propósito**: CRUD local de actividades usando SQLite.

**Funcionalidad**: Similar a `finca_local_service.dart` pero para actividades.

#### `lib/services/propietario_local_service.dart`
**Propósito**: CRUD local de propietarios usando SQLite.

**Funcionalidad**: Similar a `finca_local_service.dart` pero para propietarios.

### Modelos

#### `lib/models/propietario_model.dart`
**Propósito**: Definir estructura de datos de propietario.
...
- `synced`: bool (para control de sincronización)

**Métodos**:
- `fromJson(Map)`: Crear desde JSON
- `toJson()`: Convertir a JSON
- `toMap()`: Convertir a mapa para SQLite

### Widgets

#### `lib/widgets/finca_card.dart`
**Propósito**: Componente visual para mostrar información de una finca.
...
**Propósito**: Componente visual para mostrar actividad.
...
### Características
- ✅ **Interfaz Nativa**: Look & feel nativo en iOS y Android
- ✅ **Trabajo Offline**: CRUD completo sin conexión
- ✅ **Sincronización Inteligente**: Detecta conexión y sincroniza
- ✅ **Base de Datos Local**: SQLite para almacenamiento offline
- ✅ **Notificaciones**: Alertas de sincronización
